...
function profileVerify() {
	...
	const verifyProfileHandler = async (formData) => { 
		try {
			...
			{
				if (formData.nameFormControl == '' || formData.genderFormControl == '' || formData.emailFormControl == '' || formData.mobileFormControl == '' || formData.aadharFormControl == ''|| formData.qualiFormControl == ''|| formData.state == '' || formData.district == '' || formData.birthdateFormControl == '') {
					setError(BLANK_MSG);
					return false;
				}
				else
				{   
					...
					{
						geoAddress = geoipresponse.ip + ',' + geoipresponse.city + ',' + geoipresponse.region + ',' + geoipresponse.postal + ',' + geoipresponse.country;
						country = geoipresponse.country;
						latlong = geoipresponse.latitude + ',' + geoipresponse.longitude;
					}
					else
					{
						geoAddress = '';
					...
					{					
						...
						{
						latlong = geoipresponse['latitude'] + ',' + geoipresponse['longitude'];
							response = await verifyProfileTheory2(formData,geoAddress,latlong,settings.exam_id,userData);
						}
						...
						{
							response = await verifyProfileCreateAccount(formData,geoAddress,latlong,settings.exam_id,userData);
						}							
					}
					else
					{
						...
						response = await verifyProfile(formData,geoAddress,latlong,settings.exam_id,userData);
					}
					if(response.status == 200)
					{
						...
						if(code == 1000)
						{
							if(verifyStatus == 0) //otp page will open
							{
								var qstring = userData['usercode'] + '//@spwd++sup@//' + userData['userid'] + '//@spwd++sup@//' + userData['subuserid'] + '//@spwd++sup@//' + response.data.id_self_student+ '//@spwd++sup@//' + 5;
								var parms = window.btoa(qstring);
								navigate('/student/login-with-passcode/verify-otp?q=' + parms);
							}
							else
							{
								if(userData['examstatus'] == 0 && settings.viva_assessor == 0 && (settings.module >= 2 ) && settings.viva_first == 1)
								{
									vivaexam = 1;
									theoryexam = 0;
								}
								if(userData['examstatus'] == 0 && (settings.module >= 2 ) && settings.viva_first == 0)
								{
									vivaexam = 0;
									theoryexam = 1;
								}
								if(userData['examstatus'] == 1 && (settings.module >= 2 ) && settings.viva_first == 0)
								{
									vivaexam = 1;
									theoryexam = 0;
								}        
								if(userData['examstatus'] == 2 && settings.viva_assessor == 0 && (settings.module >= 2 ) && settings.viva_first == 1)
								{
									vivaexam = 0;
									theoryexam = 1;
								}
								if(userData['examstatus'] == 2 && settings.viva_assessor == 1 && (settings.module >= 2 ) && settings.viva_first == 1)
								{
									vivaexam = 0;
									theoryexam = 1;
								}                                     
								...
								if (settings.camera == 1 && this.globals.theoryexam == 1) {
									...
									{
										var parms = this.globals.getRandUrl('test-start');
	...
	return (
		<form
		onSubmit={handleSubmit(verifyProfileHandler)}
		>		
				...
				<div className="relative h-14 mb-3 ">
					<div>
						<input
							...
							placeholder=""
							{...register("nameFormControl", { required: true })}
						/>
						<div
				...
				<div className="relative h-14">
					<div>
						<select
							...
							{...register("genderFormControl", { required: true })}
						>
				...
				<div className="relative h-14">
					<div>
						<input
							...
							className="block pl-8 text-black pb-2.5 pt-5 w-full text-base border border-[#6E6E6E] rounded-md appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 peer"
							placeholder=""
							{...register("emailFormControl", { required: true })}
						/>
						<div
							htmlFor="floating_filled"
							className="absolute text-base text-[#1C4481] dark:text-[#1C4481] duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:text-[#1C4481] peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto flex items-center"
				...
				<div className="relative h-14">
					<div>
						<input
							...
							maxLength={10}
							placeholder=""
							{...register("mobileFormControl", { required: true })}
							 onChange={(e) => { ;
							 	setmobile(e.target.value);
							 	//if (!pattern.test(e.target.value))
							 	if (!pattern.test(e.target.value) && e.target.value.length != 10)
									setErrorForm("Invalid Mobile Number. Enter mobile number in 10 digit only.");
							 	else setErrorForm("");
							 }}					
						/>
						<div
				...
				<div className="relative h-14">
					<div>
						<input
							...
							className="block pl-8 text-black pb-2.5 pt-5 w-full text-base border border-[#6E6E6E] rounded-md appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 peer mobile_number"
							placeholder=""
							{...register("aadharFormControl", { required: true })}
						/>
						<div
							htmlFor="floating_filled"
							className="absolute text-base text-[#1C4481] dark:text-[#1C4481] duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:text-[#1C4481] peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto flex items-center"
				...
				<div className="relative h-14">
					<div>
						<select
							...
							{...register("qualiFormControl", { required: true })}
						>
							<option value="" disabled hidden>
		...
		{errorForm && <FormErrorMessage message={errorForm} setError={setErrorForm} />}
		</form>		
	);
}
