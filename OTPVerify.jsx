...
function OTPVerify() {
	...
	const otpVerifyHandler = async (formData,e) => {
		try {
			...
			{
				if (formData.otponeFormControl == '' || formData.otptwoFormControl == '' || formData.otpthreeFormControl == '' || formData.otpfourFormControl == '') {
					setError(BLANK_MSG);
					return false;
				}
				else
				{
					var otp = formData.otponeFormControl+''+formData.otptwoFormControl+''+formData.otpthreeFormControl+''+formData.otpfourFormControl;
				}
				...
				if(response.status == 200)
				{
					...
					if(code == 1000)
					{
						...
						switch(otp_type)
						{
							...
							case "3": return;
							case "4": return;
							case "5": navigate('/student/login-with-passcode/upload-id'); return;
							case '6': navigate('/student/login-with-passcode/verify-profile'); return;
